...
public class ZhongGun extends AbstractB1Card {
    ...
    {
        if (AbstractDungeon.player.hasPower("blackmythwukong:GunShi")&&AbstractDungeon.player.getPower("blackmythwukong:GunShi").amount>= GunShiUtil.getGunShiMax())
        {
            return true;
        }
        else
    ...
    {
        this.baseDamage= GunShiUtil.getGunShiDamage();
        super.applyPowers();
        if (AbstractDungeon.player.hasRelic(ShenBenYou.ID))
        {
            ifEnergy=true;
        }
        if (ifEnergy&& GunShiUtil.getGunShiMax()==10)
        {
        ...
        }else if (!ifEnergy&& GunShiUtil.getGunShiMax()==13)
        {
